        ...
        <CardContent>
          ...
          {!isAdmin && (
            <Alert severity="info" sx={{ mb: 2 }}>
              You can only generate reports for your assigned location.
            </Alert>
          )}
          
          {isAdmin && (
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Select a specific location to generate a site-specific compliance report, or choose "All Locations" for a comprehensive overview.
            </Typography>
          )}
        ...
        <>
          ...
          <Card sx={{ mb: 3 }}>
            <CardContent>
              ...
              {/* Location Coverage Alert */}
              {reportData.reportMetadata.locations.length === 1 && (
                <Alert severity="success" sx={{ mb: 2 }}>
                  <strong>Site-Specific Report:</strong> This report covers only {reportData.reportMetadata.locations[0].name}
                </Alert>
              )}
              
              {reportData.reportMetadata.locations.length > 1 && (
                <Alert severity="info" sx={{ mb: 2 }}>
                  <strong>Multi-Site Report:</strong> This report covers {reportData.reportMetadata.locations.length} locations
                </Alert>
              )}
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                    ...
                    <Typography variant="body2">
                      <strong>Period:</strong> {formatDate(reportData.reportMetadata.reportPeriod.startDate)} - {formatDate(reportData.reportMetadata.reportPeriod.endDate)}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                    <LocationIcon sx={{ mr: 1, color: 'text.secondary' }} />
                    <Typography variant="body2">
                      <strong>Location(s):</strong> {reportData.reportMetadata.locations.map(l => l.name).join(', ')}
                    </Typography>
                  </Box>
                  <Typography variant="body2" color="text.secondary">
                    <strong>Days Covered:</strong> {reportData.reportMetadata.reportPeriod.daysCovered} days
                  </Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                    ...
                    <Typography variant="body2">
                      <strong>Generated by:</strong> {reportData.reportMetadata.generatedBy.username} ({reportData.reportMetadata.generatedBy.role})
                    </Typography>
                  </Box>
                  <Typography variant="body2" color="text.secondary">
                    <strong>Generated:</strong> {formatDate(reportData.reportMetadata.generatedAt)}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    <strong>Report Type:</strong> {reportData.reportMetadata.reportType === 'full' ? 'Full Report' : 'Summary Only'}
                  </Typography>
                </Grid>
              </Grid>
            </CardContent>
